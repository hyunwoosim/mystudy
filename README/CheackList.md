...
6. finalize()
  ...
  ### iterator
1.  외부에서 구현한 패키지로 멤버클래스 사용
  - 1-1 Itrator 인터페이스를 생성
	- 1-2 ArrayListItetor 클래스로 구현
	- 1-3 Arraylist 에서 사용한다.

2. 스태틱 중첩 클래스 사용
	- 2-1 arraylist에서만 사용될 클래스라면 arraylist클래스 안에 구현하는게 유지보수하기쉽다
	- 2-2 arraylist 에서만 사용될꺼기 때문에 privat static 클래스로 변경

3. non 스태틱 클래스
  - 3-1 바깥 클래스의 인스턴스 주소를 받는 코드가 자동 추가된다. 
	- 3-2 list 의 주소를 받기 때문에 list의 관한 필드, 생성자,파라미터를 없앤다.
  - 3-3  list의 사이즈를 리턴값으로 받았던것을 Array.this.size로 바꿔준다.

4. 로컬 클래스 사용
	- 4-1 로컬 메서드안에 클래스를 넣으면 된다.
	- 4-2 로컬 클래스는 이 메서드 안에서만 사용가능하기 때문에 private을 지운다.
	- 4-3 non스태틱 클래스처럼 자동으로 인스턴스 주소를 저장하는 코드가 생성된다.
 
5. 익명클래스 사용
  - 5-1 익명클래스는 이름이 없다
  - 5-2 클래스를 정의하는 동시에 인스턴스를 생성해야한다.

    ...
    public <T> T create(Class<T> clazz, String dataName) {
        return (T) Proxy.newProxyInstance(
            ...
            (proxy, method, args) -> {
                ...
                // 서버에 요청할 때마다 연결한다.
                try (Socket sockt = new Socket(host, port);
                    DataInputStream in = new DataInputStream(sockt.getInputStream());
                    DataOutputStream out = new DataOutputStream(sockt.getOutputStream());) {
